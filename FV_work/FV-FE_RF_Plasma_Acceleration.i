...
#Argon Excited Equations
  ...
  #Diffusion term of excited Argon
  [./SM_Ar*_diffusion]
   type = FVCoeffDiffusionForShootMethod
   variable = SM_Ar*
   density = Ar*
   position_units = ${dom0Scale}
   enable = false
  ...
  [./SM_Ar*_stepwise_ionization]
    type = FVEEDFReactionLogForShootMethod
    variable = SM_Ar*
    electrons = em
    reaction = 'em + Ar* -> em + em + Ar+'
    coefficient = -1
  ...
  #Net excited Argon loss from quenching to resonant
  [./SM_Ar*_quenching]
    type = FVReactionSecondOrderLogForShootMethod
    variable = SM_Ar*
    density = Ar*
    v = em
    reaction = 'em + Ar* -> em + Ar_r'
  ...
  #Net excited Argon loss from three-body quenching
  [./SM_Ar*_3B_quenching]
    type = FVReactionThirdOrderLogForShootMethod
    variable = SM_Ar*
    density = Ar*
    v = Ar
    w = Ar
...
[MultiApps]
  ...
  [./Shooting]
    ...
    input_files = 'FV-FE_RF_Plasma_Shooting.i'
    execute_on = 'TIMESTEP_END'
    enable = false
  [../]
[]
...
[PeriodicControllers]
  [./Shooting]
    ...
    Disable_during_cycle = '*::SM_Ar*_Null'
