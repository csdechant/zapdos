...
#Helium Ion Equations
  ...
  #[../]
  [./He+_advection]
    type = EffectiveEFieldAdvectionLin
    variable = He+
    u = Ex
    v = Ey
    position_units = ${dom0Scale}
...
#Boundary conditions for electons
  ...
  [./em_He+_second_emissions]
    type = SakiyamaSecondaryElectronWithEffEfieldBCLin
    variable = em
    mean_en = mean_en
    Ex = Ex
    Ey = Ey
...
#Boundary conditions for ions
  [./He+_physical_advection]
    type = SakiyamaIonAdvectionWithEffEfieldBCLin
    variable = He+
    Ex = Ex
    Ey = Ey
    boundary = 'Top_Electrode Bottom_Electrode Top_Insulator Bottom_Insulator Walls'
...
#Boundary conditions for mean energy
  ...
  [./mean_en_He+_second_emissions]
    type = SakiyamaEnergySecondaryElectronWithEffEfieldBCLin
    variable = mean_en
    em = em
    ip = He+
    Ex = Ex
...
[Functions]
  [./potential_top_bc_func]
    type = ParsedFunction
    value = '0.200*sin(2*pi*13.56e6*t)'
  [../]
  ...
  [./energy_density_ic_func]
    ...
    value = 'if(y > 1.5 & y < 2.5,
                log((3./2.)) + log((1e10 + (-0.5 * ( tanh((x - 1.9)/0.059) - 1))*(10^15*((2.5-y)^2 * (y - 1.5)^2) + 1e13))/6.02e23),
...
[MultiApps]
  ...
  [./Shooting]
    ...
    input_files = 'He-NonLog-RF_Plasma_Shooting-2D.i'
    execute_on = 'TIMESTEP_END'
    enable = false
