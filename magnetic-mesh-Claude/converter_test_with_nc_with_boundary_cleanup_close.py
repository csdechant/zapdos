...
if __name__ == "__main__":
    ...
    n_points_inner = 100  # Adjust this to control boundary resolution
    n_points_outer = 100
    n_points_sep = 20

    print(f"Boundary discretization: {n_points_inner} points per contour")
    ...
    if contours_inner and contours_outer_wall and contours_sep:
        ...
        print(f"\nResampling contours...")
        R_inner, Z_inner = resample_contour(R_inner_raw, Z_inner_raw, n_points_inner)
        R_outer_wall, Z_outer_wall = resample_contour(R_outer_wall_raw, Z_outer_wall_raw,
                                                       n_points_outer)
        R_sep, Z_sep = resample_contour(R_sep_raw, Z_sep_raw, n_points_sep)
        ...
        def clip_boundary_to_walls(R_bound, Z_bound, R_min, R_max, Z_min, Z_max):
            """
            Remove points that are outside the wall domain.
            Keeps all points that are inside, removing only those outside.
            """
            # Find points that are inside wall boundaries
            inside_R = (R_bound >= R_min) & (R_bound <= R_max)
            inside_Z = (Z_bound >= Z_min) & (Z_bound <= Z_max)
            inside = inside_R & inside_Z

            # Keep only points inside
            R_clipped = R_bound[inside]
            Z_clipped = Z_bound[inside]

            n_removed = len(R_bound) - len(R_clipped)

            return R_clipped, Z_clipped, n_removed

        print("\nClipping boundaries to wall domain (removing points outside walls)...")

        R_inner, Z_inner, n_removed_i = clip_boundary_to_walls(
            R_inner, Z_inner, R_wall_min, R_wall_max, Z_wall_min, Z_wall_max
        )
        if n_removed_i > 0:
            print(f"  Inner: removed {n_removed_i} points outside walls")

        R_outer_wall, Z_outer_wall, n_removed_ow = clip_boundary_to_walls(
            R_outer_wall, Z_outer_wall, R_wall_min, R_wall_max, Z_wall_min, Z_wall_max
        )
        if n_removed_ow > 0:
            print(f"  Outer (wall): removed {n_removed_ow} points outside walls")

        R_sep, Z_sep, n_removed_s = clip_boundary_to_walls(
            R_sep, Z_sep, R_wall_min, R_wall_max, Z_wall_min, Z_wall_max
        )
        if n_removed_s > 0:
            print(f"  Separatrix: removed {n_removed_s} points outside walls")

        if has_divertor:
            R_outer_div, Z_outer_div, n_removed_od = clip_boundary_to_walls(
                R_outer_div, Z_outer_div, R_wall_min, R_wall_max, Z_wall_min, Z_wall_max
            )
            if n_removed_od > 0:
                print(f"  Outer (divertor): removed {n_removed_od} points outside walls")
        ...
        # Check if any boundaries were completely clipped
        if len(R_inner) == 0:
            print("  Warning: Inner boundary completely outside wall domain!")
        if len(R_outer_wall) == 0:
            print("  Warning: Outer (wall) boundary completely outside wall domain!")
        if has_divertor and len(R_outer_div) == 0:
            print("  Warning: Outer (divertor) boundary completely outside wall domain!")
            has_divertor = False  # Disable divertor export if no points remain
        ...
        if len(R_inner) > 0:
            ...
            print(f"  Inner boundary: No points (completely clipped)")
        ...
        if len(R_outer_wall) > 0:
            ...
            print(f"  Outer boundary (wall): No points (completely clipped)")
        ...
        if has_divertor and len(R_outer_div) > 0:
            ...
            print(f"  Outer boundary (divertor): No points (completely clipped)")
        ...
        wall_rect = plt.Rectangle((R_wall_min, Z_wall_min),
                                  R_wall_max - R_wall_min,
    ...
    if contours_inner and contours_outer_wall and contours_sep:
        ...
        # Check if we have enough boundaries to create a valid mesh
        if len(R_inner) == 0 and len(R_outer_wall) == 0:
            print("\nError: Cannot create Gmsh file - both inner and outer boundaries are empty!")
            print("Skipping Gmsh generation.")
        else:
            ...
            with open(geo_filename, 'w') as f:
                ...
                # Wall curve loop
                f.write("\n// Curve loops\n")
                f.write(f"Curve Loop({curve_loop_id}) = {{{', '.join(map(str, wall_line_ids))}}};\n")
                curve_loops.append(curve_loop_id)
                curve_loop_id += 1

                # Write points for inner boundary (if exists)
                if len(R_inner) > 0:
                    ...
                    f.write(f"Curve Loop({curve_loop_id}) = {{-{inner_loop_id}}};\n")
                    curve_loops.append(curve_loop_id)
                    curve_loop_id += 1

                # Write points for outer wall boundary (if exists)
                if len(R_outer_wall) > 0:
                    ...
                    f.write(f"Curve Loop({curve_loop_id}) = {{-{outer_wall_loop_id}}};\n")
                    curve_loops.insert(1, curve_loop_id)  # Insert after wall, before inner
                    curve_loop_id += 1
                ...
                if len(curve_loops) >= 2:
                    # Create surface with wall and at least one plasma boundary
                    f.write(f"Plane Surface({surface_id}) = {{{', '.join(map(str, curve_loops))}}};\n")
                    surface_id += 1
                ...
                f.write(f"\nPhysical Surface(\"plasma_region\", 1) = {{1}};\n")
            ...
            print(f"  Curves: wall", end="")
            if len(R_inner) > 0:
                print(f", inner_boundary", end="")
            if len(R_outer_wall) > 0:
                print(f", outer_wall_boundary", end="")
            if has_divertor and len(R_outer_div) > 0:
                print(f", outer_div_boundary", end="")
            print()
            print(f"  Surface: plasma_region")
