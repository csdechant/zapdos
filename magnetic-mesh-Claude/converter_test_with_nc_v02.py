...
if __name__ == "__main__":
    ...
    print("\nTracing helical field lines in 3D using Cartesian coordinates...")
    ...
    for i, (r_start, theta_s, color) in enumerate(zip(r_start_values, theta_start, colors)):
        ...
        # Arrays to store field line in Cartesian coordinates
        x_line = np.zeros(n_points)
        ...
        # Trace field line using Cartesian field components
        # dx/ds = B_x/|B|, dy/ds = B_y/|B|, dz/ds = B_z/|B|
        # where s is arc length along field line
        for j in range(1, n_points):
            # Get Cartesian field at current position
            B_x_curr, B_y_curr, B_z_curr, B_mag_curr = mag_field.get_cartesian_field(
                x_line[j-1], y_line[j-1], z_line[j-1]
            )
            ...
            # Normalize field to unit vector (direction of field line)
            b_x = B_x_curr / B_mag_curr
            b_y = B_y_curr / B_mag_curr
            b_z = B_z_curr / B_mag_curr

            # Step length (choose based on characteristic length scale)
            ds = 0.02  # Step size in meters

            # Euler step along field line
            x_line[j] = x_line[j-1] + b_x * ds
            y_line[j] = y_line[j-1] + b_y * ds
            z_line[j] = z_line[j-1] + b_z * ds

            # Check if we've completed the desired toroidal turns
            # Calculate current toroidal angle
            phi_current = np.arctan2(y_line[j], x_line[j])
            phi_previous = np.arctan2(y_line[j-1], x_line[j-1])

            # Handle angle wrapping
            if phi_current - phi_previous < -np.pi:
                phi_current += 2*np.pi
            elif phi_current - phi_previous > np.pi:
                phi_current -= 2*np.pi

            # Accumulate total toroidal angle traveled
            if j == 1:
                total_phi = 0
            else:
                total_phi += (phi_current - phi_previous)

            if abs(total_phi) > 2*np.pi*toroidal_turns:
                x_line = x_line[:j+1]
                y_line = y_line[:j+1]
                z_line = z_line[:j+1]
                break

        # Plot field line (using Cartesian coordinates directly!)
    ...
    print(f"  Traced {n_fieldlines} field lines using Cartesian field components")
    print(f"  Each field line follows B = (B_x, B_y, B_z) for {toroidal_turns} toroidal turns")
    ...
    # =========================================================================
    # Export magnetic field data to file
    # =========================================================================
    print("\n" + "=" * 50)
    print("Exporting magnetic field data...")
    print("=" * 50)

    # Define ranges and number of points for export
    R_range = (0.5289, 1.2775)  # Major radius range (m)
    Z_range = (-1.1161, 0.8367)  # Vertical range (m)
    phi_range = (0.0, np.pi/4)  # Toroidal angle range (rad)
    ...
    print(f"\nExport parameters:")
    print(f"  R range: {R_range[0]:.2f} to {R_range[1]:.2f} m ({n_R} points)")
    print(f"  Z range: {Z_range[0]:.2f} to {Z_range[1]:.2f} m ({n_Z} points)")
    print(f"  φ range: {phi_range[0]:.2f} to {phi_range[1]:.2f} rad ({n_phi} points)")
    print(f"  Total points: {n_R * n_Z * n_phi}")
    ...
    for phi_val in phi_export:
        for Z_val in Z_export:
            for R_val in R_export:
                ...
                try:
                    B_x_val, B_y_val, B_z_val, B_mag_val = mag_field.get_cartesian_field(
                        x_val, y_val, z_val
                    )

                    # Also get psi value at this location
                    psi_val = mag_field.psi_interp(Z_val, R_val, grid=False)

                    # Store data: x, y, z, psi, Bx, By, Bz
                    data_list.append([
                        x_val, y_val, z_val,
                        psi_val,
                    ...
                    # Skip points outside interpolation domain
                    continue
    ...
    # Save to CSV file
    header = "x(m),y(m),z(m),psi(Wb),Bx(T),By(T),Bz(T)"
    filename = 'magnetic_field_data.csv'
    np.savetxt(filename, data_array, delimiter=',', header=header, comments='')

    print(f"\nData exported to '{filename}'")
    print(f"  Total valid points: {len(data_list)}")
    print(f"  Columns: {header}")

    # Print sample of the data
    print("\n" + "-" * 50)
    print("Sample of exported data (first 10 points):")
    print("-" * 50)
    print(f"{'x(m)':>10} {'y(m)':>10} {'z(m)':>10} {'ψ(Wb)':>12} "
          f"{'Bx(T)':>10} {'By(T)':>10} {'Bz(T)':>10}")
    print("-" * 50)
    for i in range(min(10, len(data_array))):
        row = data_array[i]
        print(f"{row[0]:10.4f} {row[1]:10.4f} {row[2]:10.4f} {row[3]:12.6f} "
              f"{row[4]:10.6f} {row[5]:10.6f} {row[6]:10.6f}")

    print("\n" + "=" * 50)
    print("Export complete!")
    print("=" * 50)
    print(f"\nFile created: {filename}")
    print(f"Format: CSV with {len(data_array)} data points")
    print(f"Columns: x(m), y(m), z(m), psi(Wb), Bx(T), By(T), Bz(T)")
