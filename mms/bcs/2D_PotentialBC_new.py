...
#Know solutions
ne = '(sin(pi*y) + 0.2*sin(2*pi*t)*cos(pi*y) + 2.0 + sin(pi*x)) / N_A'
ni = '(sin(pi*y) + 0.2*sin(2*pi*t)*cos(pi*y) + 2.0 + sin(pi*x)) / N_A'

V = '(sin(pi*t)/pi + 2)*(sin(pi*y) + sin(pi*x))'
Ex = '-pi*cos(pi*x)*(sin(pi*t) + 1) - 3'
Ey = '-cos(pi*y)*(sin(pi*t) + 1) - 3'

nenergy = '(3*massem*pi*(2*muion*(cos(pi*y)*(sin(pi*t) + 1) + 3)*(sin(pi*x) + sin(pi*y) + (cos(pi*y)*sin(2*pi*t))/5 + 2) - (cos(pi*t)*(sin(pi*x) + sin(pi*y)))/ee + (diffpotential*pi*cos(pi*t)*cos(pi*y))/ee)*(2*muion*(cos(pi*y)*(sin(pi*t) + 1) + 3)*(sin(pi*x) + sin(pi*y) + (cos(pi*y)*sin(2*pi*t))/5 + 2) - (cos(pi*t)*(sin(pi*x) + sin(pi*y)))/ee + (diffpotential*pi*cos(pi*t)*cos(pi*y))/ee))/(ee*(sin(pi*x) + sin(pi*y) + (cos(pi*y)*sin(2*pi*t))/5 + 2))'
...
fenergy,senergy = mms.evaluate('diff(nenergy,t) + div(-diffmean_en * grad(nenergy))',
                     ...
                     scalars=['N_A','ee','diffpotential','diffion','muion','diffem','muem','massem','diffmean_en','x_max','y_max','f'])
