...
#Source Terms in moles
  ...
  [./int_value]
    ...
    vars = 't_old'
    vals = 't_old'
    value = 't_old - t + (sin(pi*t)/(4*pi) + 1/4)*(sin(pi*x) + sin(pi*y)) - (sin(pi*t_old)/(4*pi) + 1/4)*(sin(pi*x) + sin(pi*y)) - (pi*cos(pi*y)*(sin(pi*t)/(4*pi) + 1/4))/20 + (pi*cos(pi*y)*(sin(pi*t_old)/(4*pi) + 1/4))/20'
  [../]
[]
