#!/usr/bin/env python3

import mms

#Know solutions
ne = '(sin(pi*y) + 0.2*sin(2*pi*t)*cos(pi*y) + 2.0 + sin(pi*x)) / N_A'
ni = '(sin(pi*y) + 0.2*sin(2*pi*t)*cos(pi*y) + 2.0 + sin(pi*x)) / N_A'

V = '0.25*(sin(pi*t)/pi + 1.0)*(sin(pi*y) + sin(pi*x)) - t'
Ex = '-pi*cos(pi*x)*(sin(pi*t) + 1)'
Ey = '-pi*cos(pi*y)*(sin(pi*t) + 1)'

energy = '(6*pi*((pi*cos(pi*t)*cos(pi*y))/80 - (cos(pi*t)*(sin(pi*x) + sin(pi*y)))/4 + 2*pi*cos(pi*y)*(sin(pi*t) + 1)*(sin(pi*x)/100 + sin(pi*y)/100 + (cos(pi*y)*sin(2*pi*t))/500 + 1/50) + 1)*((pi*cos(pi*t)*cos(pi*y))/80 - (cos(pi*t)*(sin(pi*x) + sin(pi*y)))/4 + 2*pi*cos(pi*y)*(sin(pi*t) + 1)*(sin(pi*x)/100 + sin(pi*y)/100 + (cos(pi*y)*sin(2*pi*t))/500 + 1/50) + 1))/((sin(x*pi) + sin(y*pi) + (cos(y*pi)*sin(2*t*pi))/5 + 2)*(sin(x*pi) + sin(y*pi) + (cos(y*pi)*sin(2*t*pi))/5 + 2))'
nenergy = '('+energy+')'+'*'+'('+ne+')'

#Ion Diffusion-Advection Problem
fem,sem = mms.evaluate('diff(ne,t) + div(-diffem * grad(ne) - muem*ne*-grad(V))',
                     ne, variable='ne',
                     V=V,
                     scalars=['N_A','ee','diffem','muem','diffpotential','x_max','y_max','f'])

fion,sion = mms.evaluate('diff(ni,t) + div(-diffion * grad(ni) + muion*ni*(Ex*e_i + Ey*e_j))',
                     ni, variable='ni',
                     Ex=Ex,
                     Ey=Ey,
                     scalars=['N_A','ee','diffion','muion','diffpotential','x_max','y_max','f'])

fV,sV = mms.evaluate('diff(V,t) + div(-diffpotential * grad(V))',
                     V, variable='V',
                     ne=ne,
                     ni=ni,
                     scalars=['N_A','ee','diffpotential','x_max','y_max','f'])


fEx,sEx = mms.evaluate('diff(Ex,t) + div(-diffpotential * grad(Ex))',
                     Ex, variable='Ex',
                     scalars=['N_A','ee','diffpotential','x_max','y_max','f'])


fEy,sEy = mms.evaluate('diff(Ey,t) + div(-diffpotential * grad(Ey))',
                     Ey, variable='Ey',
                     scalars=['N_A','ee','diffpotential','x_max','y_max','f'])

fenergy,senergy = mms.evaluate('diff(nenergy,t) + div(-diffmean_en * grad(nenergy))',
                     nenergy, variable='nenergy',
                     scalars=['N_A','ee','diffpotential','diffion','muion','diffem','muem','massem','diffmean_en','x_max','y_max','f'])


mms.print_hit(fem, 'em_source')
mms.print_hit(fion, 'ion_source')
mms.print_hit(fenergy, 'energy_source')
mms.print_hit(fEx, 'Ex_source')
mms.print_hit(fEy, 'Ey_source')
mms.print_hit(fV, 'potential_source')
