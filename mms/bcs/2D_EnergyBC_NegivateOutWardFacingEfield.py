#!/usr/bin/env python3

import mms

#Know solutions
ne = '((16*ee*(sin(pi*x) + sin(pi*y) + (cos(pi*y)*sin(2*pi*t))/5 + 1)*(sin(pi*x) + sin(pi*y) + (cos(pi*y)*sin(2*pi*t))/5 + 1)*(sin(pi*x) + sin(pi*y) + (cos(pi*y)*sin(2*pi*t))/5 + 1))/(3*massem*pi*((12*diffmean_en*pi)/5 + (12*mumean_en*pi*(sin(t*pi) + 1)*(sin(x*pi) + sin(y*pi) + (cos(y*pi)*sin(2*t*pi))/5 + 1))/5)*((12*diffmean_en*pi)/5 + (12*mumean_en*pi*(sin(t*pi) + 1)*(sin(x*pi) + sin(y*pi) + (cos(y*pi)*sin(2*t*pi))/5 + 1))/5))) / N_A'
ni = '(sin(pi*y) + 0.2*sin(2*pi*t)*cos(pi*y) + 1.0 + sin(pi*x)) / N_A'

V = '-(sin(pi*t) + 1.0)*(sin(pi*y) + sin(pi*x))'
Ex = 'pi*cos(pi*x)*(sin(pi*t) + 1)'
Ey = 'pi*cos(pi*y)*(sin(pi*t) + 1)'

nenergy = 'sin(pi*y) + 0.2*sin(2*pi*t)*cos(pi*y) + 1.0 + sin(pi*x)'

#Ion Diffusion-Advection Problem
fem,sem = mms.evaluate('diff(ne,t) + div(-diffem * grad(ne))',
                     ne, variable='ne',
                     V=V,
                     scalars=['N_A','massem','ee','diffem','muem','diffmean_en','mumean_en','diffpotential','x_max','y_max','f'])

fion,sion = mms.evaluate('diff(ni,t) + div(-diffion * grad(ni) + muion*ni*(Ex*e_i + Ey*e_j))',
                     ni, variable='ni',
                     Ex=Ex,
                     Ey=Ey,
                     scalars=['N_A','ee','diffion','muion','diffpotential','x_max','y_max','f'])

fV,sV = mms.evaluate('diff(V,t) + div(-diffpotential * grad(V))',
                     V, variable='V',
                     scalars=['N_A','ee','diffpotential','x_max','y_max','f'])


fEx,sEx = mms.evaluate('diff(Ex,t) + div(-diffpotential * grad(Ex))',
                     Ex, variable='Ex',
                     scalars=['N_A','ee','diffpotential','x_max','y_max','f'])


fEy,sEy = mms.evaluate('diff(Ey,t) + div(-diffpotential * grad(Ey))',
                     Ey, variable='Ey',
                     scalars=['N_A','ee','diffpotential','x_max','y_max','f'])

fenergy,senergy = mms.evaluate('diff(nenergy,t) + div(-diffmean_en * grad(nenergy) - mumean_en*nenergy*-grad(V))',
                     nenergy, variable='nenergy',
                     V=V,
                     scalars=['N_A','ee','diffem','muem','massem','diffmean_en','mumean_en','x_max','y_max','f'])


mms.print_hit(fem, 'em_source')
mms.print_hit(fion, 'ion_source')
mms.print_hit(fenergy, 'energy_source')
mms.print_hit(fEx, 'Ex_source')
mms.print_hit(fEy, 'Ey_source')
mms.print_hit(fV, 'potential_source')
