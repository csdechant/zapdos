...
[Functions]
  [potential_bc_func]
    ...
    # expression = '-125 * sin(2*pi*13.56e6*t)'
    # expression = '-175 * sin(2*pi*13.56e6*t)'

    # expression = 'if(t < 8.88e-5, -1.0 * (175 + 125 * (t - 7.4e-5)/(14.8e-5 - 7.4e-5)) * sin(2*pi*13.56e6*t) , -200 * sin(2*pi*13.56e6*t))'
    # expression = '-200 * sin(2*pi*13.56e6*t)'
    expression = '-1.0 * (200 + 100 * (t - 14.1076e-5)/(21.4822e-5 - 14.1076e-5)) * sin(2*pi*13.56e6*t)'
  []
  [potential_gap_func]
    type = ParsedFunction
    symbol_names = 'plate'
