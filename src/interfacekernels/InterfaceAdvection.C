...
validParams<InterfaceAdvection>()
{
  ...
  params.addRequiredParam<Real>("neighbor_time_units",
                                "The units of time in the neighboring domain.");
...
InterfaceAdvection::InterfaceAdvection(const InputParameters & parameters)
  : InterfaceKernel(parameters),
    ...
    _time_neighbor_units(1. / getParam<Real>("neighbor_time_units")),
...
InterfaceAdvection::computeQpResidual(Moose::DGResidualType type)
{
  ...
  {
    case Moose::Element:
      r = _mu_neighbor[_qp] * _time_neighbor_units * _sgn_neighbor[_qp] * -_grad_potential_neighbor[_qp] *
...
InterfaceAdvection::computeQpJacobian(Moose::DGJacobianType type)
{
  ...
  switch (type)
  {
    case Moose::ElementNeighbor:
      ...
      jac += _mu_neighbor[_qp] * _time_neighbor_units * _sgn_neighbor[_qp] * -_grad_potential_neighbor[_qp] *
                 _r_neighbor_units * std::exp(_neighbor_value[_qp]) * _phi_neighbor[_j][_qp] *
                 _normals[_qp] * _test[_i][_qp] * _r_units +
             _d_mu_neighbor_d_actual_mean_en[_qp] * _time_neighbor_units * _actual_mean_en * -_phi_neighbor[_j][_qp] *
...
InterfaceAdvection::computeQpOffDiagJacobian(Moose::DGJacobianType type, unsigned int jvar)
{
  ...
  if (jvar == _potential_neighbor_id)
  {
    switch (type)
    {
      ...
      case Moose::ElementNeighbor:
        r += (_mu_neighbor[_qp] * _time_neighbor_units * _sgn_neighbor[_qp] * -_grad_phi_neighbor[_j][_qp] *
  ...
  else if (jvar == _mean_en_neighbor_id)
  {
    switch (type)
    {
      ...
      case Moose::ElementNeighbor:
        _actual_mean_en = std::exp(_mean_en_neighbor[_qp] - _neighbor_value[_qp]);
        r += (_d_mu_neighbor_d_actual_mean_en[_qp] * _time_neighbor_units * _actual_mean_en * _phi_neighbor[_j][_qp] *
              _sgn_neighbor[_qp] * -_grad_potential_neighbor[_qp] * _r_neighbor_units *
