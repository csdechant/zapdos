...
#include "FVSakiyamaElectronDiffusionBCNonLog.h"
#include "Function.h"

registerMooseObject("ZapdosApp", FVSakiyamaElectronDiffusionBCNonLog);

InputParameters
FVSakiyamaElectronDiffusionBCNonLog::validParams()
{
...
FVSakiyamaElectronDiffusionBCNonLog::FVSakiyamaElectronDiffusionBCNonLog(const InputParameters & parameters)
  : FVFluxBC(parameters),
...
ADReal
FVSakiyamaElectronDiffusionBCNonLog::computeQpResidual()
{
  using namespace Moose::FV;
  ...
  interpolate(
      ...
      energy_interface,
      (_mean_en[_qp] / _u[_qp]),
      (_mean_en_neighbor[_qp] / _u_neighbor[_qp]),
      *_face_info,
      true);
  ...
  return _r_units * (0.25 * _v_thermal * _u[_qp]);
}
