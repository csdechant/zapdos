...
FVElectronTemperatureDirichletBC::FVElectronTemperatureDirichletBC(
...
FVElectronTemperatureDirichletBC::boundaryValue(const FaceInfo & fi) const
{
  ...
  ADReal u_face;
  interpolate(InterpMethod::Average, u_face, _u[0], _u_neighbor[0], fi, true);

  ADReal em_face;
  interpolate(InterpMethod::Average, em_face, _em[0], _em_neighbor[0], fi, true);
