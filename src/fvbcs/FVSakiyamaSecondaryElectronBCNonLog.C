...
#include "FVSakiyamaSecondaryElectronBCNonLog.h"
#include "Function.h"

registerMooseObject("ZapdosApp", FVSakiyamaSecondaryElectronBCNonLog);

InputParameters
FVSakiyamaSecondaryElectronBCNonLog::validParams()
{
...
FVSakiyamaSecondaryElectronBCNonLog::FVSakiyamaSecondaryElectronBCNonLog(const InputParameters & parameters)
  : FVFluxBC(parameters),
    ...
    _user_se_coeff(getParam<Real>("users_gamma"))
{
  ...
  for (unsigned int i = 0; i < _num_ions; ++i)
  {
    ...
    _muion_neighbor[i] = &getNeighborADMaterialProperty<Real>("mu" + (*getFieldVar("ip", i)).name());
...
ADReal
FVSakiyamaSecondaryElectronBCNonLog::computeQpResidual()
{
  ...
  for (unsigned int i = 0; i < _num_ions; ++i)
  {
    ...
    _ion_flux += _a * (*_sgnion[i])[_qp] * muion * -grad_potential * _r_units *
                 ion_face;
  }

  return -1.0 * _r_units * _a * _user_se_coeff * _ion_flux * _normal;
}
