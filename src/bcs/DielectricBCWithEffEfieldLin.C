...
#include "DielectricBCWithEffEfieldLin.h"

registerMooseObject("ZapdosApp", DielectricBCWithEffEfieldLin);

InputParameters
DielectricBCWithEffEfieldLin::validParams()
{
...
DielectricBCWithEffEfieldLin::DielectricBCWithEffEfieldLin(const InputParameters & parameters)
...
ADReal
DielectricBCWithEffEfieldLin::computeQpResidual()
{
  ...
  _ion_flux =
      (_a * _sgnip[_qp] * _muip[_qp] * EField * _r_units * _ip[_qp]);

  _v_thermal =
      std::sqrt(8 * _e[_qp] * 2.0 / 3 * (_mean_en[_qp] / _em[_qp]) / (M_PI * _massem[_qp]));

  _em_flux =
      (0.25 * _v_thermal * _em[_qp] * _normals[_qp]) - (_user_se_coeff * _ion_flux);
  ...
  _ion_flux_old =
      (_a_old * _sgnip[_qp] * _muip[_qp] * EField_old * _r_units * _ip_old[_qp]);


  _v_thermal_old =
      std::sqrt(8 * _e[_qp] * 2.0 / 3 * (_mean_en_old[_qp] / _em_old[_qp]) / (M_PI * _massem[_qp]));

  _em_flux_old =
      (0.25 * _v_thermal_old * _em_old[_qp] * _normals[_qp]) - (_user_se_coeff * _ion_flux_old);
