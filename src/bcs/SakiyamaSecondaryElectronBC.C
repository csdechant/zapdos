...
SakiyamaSecondaryElectronBC::computeQpResidual()
{
  ...
  _ion_flux = _a * _sgnip[_qp] * _muip[_qp] * -_grad_potential[_qp] * _r_units * std::exp(_ip[_qp]);

  return -_test[_i][_qp] * _r_units * _user_se_coeff * _ion_flux * _normals[_qp];
}
...
SakiyamaSecondaryElectronBC::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _potential_id)
  {
    ...
    return -_test[_i][_qp] * _r_units * _user_se_coeff * _d_ion_flux_d_potential *
           _normals[_qp];
  }
