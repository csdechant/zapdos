...
#include "EconomouDielectricBC_Rearranged.h"

#include "MooseVariable.h"

registerMooseObject("ZapdosApp", EconomouDielectricBC_Rearranged);

template <>
InputParameters
validParams<EconomouDielectricBC_Rearranged>()
{
...
EconomouDielectricBC_Rearranged::EconomouDielectricBC_Rearranged(const InputParameters & parameters)
...

{
  ...
  if (coupledComponents("potential_ion") != _num_ions)
  {
    ...
    {
      mooseError(
          "EconomouDielectricBC_Rearranged: ip and potential_ion vectors are not same length. There are two "
...
Real
EconomouDielectricBC_Rearranged::computeQpResidual()
{
  _ion_flux.zero();
  for (unsigned int i = 0; i < _num_ions; ++i)
  {
  ...
  return _test[_i][_qp] * _r_units *
         (_e[_qp] * 6.022e23 * (_ion_flux - _em_flux) * _normals[_qp] /
              _voltage_scaling +
...
Real
EconomouDielectricBC_Rearranged::computeQpJacobian()
{
  _d_ion_flux_du.zero();
  ...
  return _test[_i][_qp] * _r_units *
         (_e[_qp] * 6.022e23 * _d_ion_flux_du * _normals[_qp] /
              _voltage_scaling +
          8.854e-12 * _du_dot_du[_qp] * -_grad_phi[_j][_qp] * _r_units *
              _normals[_qp] -
          (_epsilon_d / _thickness) * _du_dot_du[_qp] * _phi[_j][_qp]);
}

Real
EconomouDielectricBC_Rearranged::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  else if (_iter != _ion_id.end())
  {
    ...
    return _test[_i][_qp] * _r_units * _e[_qp] * 6.022e23 *
