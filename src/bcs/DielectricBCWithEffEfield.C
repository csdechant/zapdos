...
DielectricBCWithEffEfield::validParams()
{
  ...
  params.addRequiredParam<Real>("dielectric_constant", "The dielectric constant (relative permittivity) of the material.");
  ...
  params.addRequiredParam<FunctionName>("int_value", "The function.");
  params.addClassDescription("The integral version of the dielectric boundary condition, "
                             "requires E-field components");
  return params;
}
...
DielectricBCWithEffEfield::DielectricBCWithEffEfield(const InputParameters & parameters)
  : ADIntegratedBC(parameters),
    ...
    _int_value(getFunction("int_value")),

    _u_old(_var.slnOld()),
    _grad_u_old(_var.gradSlnOld()),

    _mean_en(adCoupledValue("mean_en")),
    _mean_en_old(coupledValueOld("mean_en")),

    _em(adCoupledValue("em")),
    _em_old(coupledValueOld("em")),

    _ip(adCoupledValue("ip")),
    _ip_var(*getVar("ip", 0)),
    _ip_old(coupledValueOld("ip")),


    _Ex(adCoupledValue("Ex")),
    _Ex_old(coupledValueOld("Ex")),

    _Ey(isCoupled("Ey") ? adCoupledValue("Ey") : _ad_zero),
    _Ey_old(isCoupled("Ey") ? coupledValueOld("Ey") : _zero),

    _Ez(isCoupled("Ez") ? adCoupledValue("Ez") : _ad_zero),
    _Ez_old(isCoupled("Ez") ? coupledValueOld("Ez") : _zero),
...
DielectricBCWithEffEfield::computeQpResidual()
{
  ...
  return _test[_i][_qp] * _r_units * (-_epsilon_0[_qp] * _grad_u_old[_qp] * _normals[_qp] +
              (_epsilon_d * 8.8542e-12 / _thickness) * (_u[_qp] - _u_old[_qp]) - _int * _normals[_qp]);

  //return _test[_i][_qp] * _r_units * (-_epsilon_0[_qp] * _grad_u_old[_qp] * _normals[_qp] +
  //            (_epsilon_d * 8.8542e-12 / _thickness) * (_u[_qp] - _u_old[_qp]) - _int_value.value(_t, _q_point[_qp]));
}
