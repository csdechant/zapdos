#include "ZapdosEEDFLinearInterpolation.h"
#include "MooseUtils.h"

// MOOSE includes
#include "MooseVariable.h"

registerMooseObject("ZapdosApp", ZapdosEEDFLinearInterpolation);

template <>
InputParameters
validParams<ZapdosEEDFLinearInterpolation>()
{
...
ZapdosEEDFLinearInterpolation::ZapdosEEDFLinearInterpolation(const InputParameters & parameters)
  : Material(parameters),
    ...
    _mean_en(isCoupled("mean_energy") ? coupledValue("mean_energy") : _zero)
{
  if (!isParamValid("sampler") && !isParamValid("mean_energy"))
    mooseError("Material ZapdosEEDFLinearInterpolation requires either a sampling variable or the "
...
void
ZapdosEEDFLinearInterpolation::computeQpProperties()
{
