#include "GasElectronMoments2.h"
#include "MooseUtils.h"

registerMooseObject("ZapdosApp", GasElectronMoments2);

template <>
InputParameters
validParams<GasElectronMoments2>()
{
...
GasElectronMoments2::GasElectronMoments2(const InputParameters & parameters)
  : Material(parameters),
...
void
GasElectronMoments2::computeQpProperties()
{
  ...
  if (_interp_trans_coeffs)
  {
    if (_ramp_trans_coeffs)
    {
      ...
      _muem[_qp] =
          ...
          _voltage_scaling * _time_units / (_n_gas[_qp] * _N_A[_qp]);
      _d_muem_d_actual_mean_en[_qp] =
          std::tanh(_t / 1e-6) *
          _mu_interpolation.sampleDerivative(std::exp(_mean_en[_qp] - _em[_qp])) * _voltage_scaling * _time_units / (_n_gas[_qp] * _N_A[_qp]);
      _diffem[_qp] =
          std::tanh(_t / 1e-6) * _diff_interpolation.sample(std::exp(_mean_en[_qp] - _em[_qp])) +
          (1. - std::tanh(_t / 1e-6)) * .30 * _time_units / (_n_gas[_qp] * _N_A[_qp]);
    ...
    else
    {
      if (_use_moles)
      {
      _muem[_qp] = _mu_interpolation.sample(std::exp(_mean_en[_qp] - _em[_qp])) * _voltage_scaling * _time_units / (_n_gas[_qp] * _N_A[_qp]);
      _d_muem_d_actual_mean_en[_qp] =
          _mu_interpolation.sampleDerivative(std::exp(_mean_en[_qp] - _em[_qp])) * _voltage_scaling * _time_units / (_n_gas[_qp] * _N_A[_qp]);
      _diffem[_qp] = _diff_interpolation.sample(std::exp(_mean_en[_qp] - _em[_qp])) * _time_units / (_n_gas[_qp] * _N_A[_qp]);
      _d_diffem_d_actual_mean_en[_qp] =
          _diff_interpolation.sampleDerivative(std::exp(_mean_en[_qp] - _em[_qp])) * _time_units / (_n_gas[_qp] * _N_A[_qp]);
      }
      else
