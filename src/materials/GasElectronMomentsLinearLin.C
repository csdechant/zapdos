...
#include "GasElectronMomentsLinearLin.h"
#include "MooseUtils.h"

registerMooseObject("ZapdosApp", GasElectronMomentsLinearLin);

InputParameters
GasElectronMomentsLinearLin::validParams()
{
...
GasElectronMomentsLinearLin::GasElectronMomentsLinearLin(const InputParameters & parameters)
  : Material(parameters),
    ...
    _mean_en(isCoupled("mean_en") ? adCoupledValue("mean_en") : _ad_zero)
{
  ...
  if (myfile.is_open())
  {
    ...
    _mu_interpolation =
      libmesh_make_unique<LinearInterpolation>(actual_mean_energy, mu, true);
    _diff_interpolation =
      libmesh_make_unique<LinearInterpolation>(actual_mean_energy, diff, true);
}

void
GasElectronMomentsLinearLin::computeQpProperties()
{
  ...
  if (_pressure_dependent) // If the mobility and diff. does depend on pressure
  {
    ...
    if (_interp_trans_coeffs)
    {
      if (_ramp_trans_coeffs)
      {
        _muem[_qp].value() =
            (std::tanh(_t / 1e-6) *
                 _mu_interpolation->sample((_mean_en[_qp].value() / _em[_qp].value())) +
             (1. - std::tanh(_t / 1e-6)) * .0352) *
            _voltage_scaling * _time_units * _N_inverse;
        _muem[_qp].derivatives() =
            std::tanh(_t / 1e-6) *
            _mu_interpolation->sampleDerivative((_mean_en[_qp].value() / _em[_qp].value())) *
            ((_mean_en[_qp].derivatives() / _em[_qp].value()) -
             (_em[_qp].derivatives() * _mean_en[_qp].value() /
                (_em[_qp].value() * _em[_qp].value()))) *
            _voltage_scaling * _time_units * _N_inverse;
        _diffem[_qp].value() =
            std::tanh(_t / 1e-6) *
                _diff_interpolation->sample((_mean_en[_qp].value() / _em[_qp].value())) +
    ...
    if (_interp_trans_coeffs)
    {
      if (_ramp_trans_coeffs)
      {
        ...
        _muem[_qp].derivatives() =
            ...
            ((_mean_en[_qp].derivatives() / _em[_qp].value()) -
             (_em[_qp].derivatives() * _mean_en[_qp].value() /
                (_em[_qp].value() * _em[_qp].value()))) * _voltage_scaling * _time_units;
        _diffem[_qp].value() =
            std::tanh(_t / 1e-6) *
                _diff_interpolation->sample((_mean_en[_qp].value() / _em[_qp].value())) +
  ...
  _TemVolts[_qp] = 2. / 3. * (_mean_en[_qp] / _em[_qp]);
