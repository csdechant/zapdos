...
PlasmaEMProperties::computeQpProperties()
{
  ...
  _eps_r_real[_qp] =
      1.0 - (pow(omega_pe, 2) / (pow(2 * _pi * _frequency, 2) + pow(2 * _pi * _nu[_qp], 2)));
  _eps_r_imag[_qp] =
      (-1.0 * pow(omega_pe, 2) * 2 * _pi * _nu[_qp]) /
      (pow(2 * _pi * _frequency, 3) + 2 * _pi * _frequency * pow(2 * _pi * _nu[_qp], 2));

  // Calculate the gradient of the plasma dielectric constant
  ADReal grad_const =
      -pow(omega_pe, 2) / (pow(2 * _pi * _frequency, 2) + pow(2 * _pi * _nu[_qp], 2));
  _eps_r_real_grad[_qp] = grad_const * _em_grad[_qp];
  _eps_r_imag_grad[_qp] =
      (grad_const * 2 * _pi * _nu[_qp] / (2 * _pi * _frequency)) * _em_grad[_qp];
  ...
  if (_fe_problem.isTransient())
  {
    ...
    _eps_r_real_dot[_qp] = -1.0 * pow(omega_pe_const, 2) * lin_dot /
                           (pow(2 * _pi * _frequency, 2) + pow(2 * _pi * _nu[_qp], 2));

    _eps_r_imag_dot[_qp] =
        -1.0 * pow(omega_pe_const, 2) * 2 * _pi * _nu[_qp] * lin_dot /
        (pow(2 * _pi * _frequency, 3) + 2 * _pi * _frequency * pow(2 * _pi * _nu[_qp], 2));

    _sigma_pe_real[_qp] =
        pow(omega_pe, 2) * _nu[_qp] / (pow(2 * _pi * _frequency, 2) + pow(_nu[_qp], 2));
    _sigma_pe_imag[_qp] = -1.0 * pow(omega_pe, 2) * 2 * _pi * _frequency /
                          (pow(2 * _pi * _frequency, 2) + pow(_nu[_qp], 2));
     ...
     *        /test/tests/mms/materials/2D_PlasmaEMProperties.i.
     *
    ...
    ADReal lin_dot_dot = _em_dot_dot[_qp] * exp(_em[_qp]) + pow(_em_dot[_qp], 2) * exp(_em[_qp]);
    ...
    _eps_r_imag_dot_dot[_qp] =
        -1.0 * pow(omega_pe_const, 2) * 2 * _pi * _nu[_qp] * lin_dot_dot /
        (pow(2 * _pi * _frequency, 3) + 2 * _pi * _frequency * pow(2 * _pi * _nu[_qp], 2));
  }
}
