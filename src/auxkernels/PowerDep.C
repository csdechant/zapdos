...
PowerDep::computeValue()
{
  _current =
      ...
      (_sgn[_qp] * (_mu[_qp] / _time_units) * -_grad_potential[_qp] * _r_units * std::exp(_density_log[_qp]) -
       (_diff[_qp] / _time_units) * std::exp(_density_log[_qp]) * _grad_density_log[_qp] * _r_units);
