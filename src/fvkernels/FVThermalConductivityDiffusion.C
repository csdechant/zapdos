...
#include "FVThermalConductivityDiffusion.h"
#include "Assembly.h"
...
registerADMooseObject("ZapdosApp", FVThermalConductivityDiffusion);

InputParameters
FVThermalConductivityDiffusion::validParams()
{
...
FVThermalConductivityDiffusion::FVThermalConductivityDiffusion(const InputParameters & params)
  : FVFluxKernel(params),
...
ADReal
FVThermalConductivityDiffusion::computeQpResidual()
{

  using namespace Moose::FV;
  const auto state = determineState();

  auto grad_u = gradUDotNormal(state);
  ADRealVectorValue grad_em = adCoupledGradientFace("em", *_face_info, state);

  using namespace Moose::FV;

  ADReal diffem;
  interpolate(
      InterpMethod::Average, diffem, _diffem_elem[_qp], _diffem_neighbor[_qp], *_face_info, true);
  ...
  return _coeff * diffem *
         (u_interface * grad_u * _r_units -
          (u_interface / em_interface) * em_interface * grad_em * _normal * _r_units);
}
