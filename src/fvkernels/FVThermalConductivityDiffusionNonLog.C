...
FVThermalConductivityDiffusionNonLog::FVThermalConductivityDiffusionNonLog(
...
FVThermalConductivityDiffusionNonLog::computeQpResidual()
{
  ...
  ADReal u_interface;
  interpolate(
      InterpMethod::Average, u_interface, _u_elem[_qp], _u_neighbor[_qp], *_face_info, true);

  ADReal em_interface;
  interpolate(
      InterpMethod::Average, em_interface, _em_elem[_qp], _em_neighbor[_qp], *_face_info, true);

  // Is _r_units needed like for _grad_test[_i][_qp]???
  return _coeff * diffem *
         (grad_u * _r_units - (u_interface / em_interface) * grad_em * _normal * _r_units);
}
