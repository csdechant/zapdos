...
#include "FVEEDFElastic.h"

registerMooseObject("ZapdosApp", FVEEDFElastic);

InputParameters
FVEEDFElastic::validParams()
{
...
FVEEDFElastic::FVEEDFElastic(const InputParameters & parameters)
...
ADReal
FVEEDFElastic::computeQpResidual()
{
  ADReal Eel = -3.0 * _massem / _massGas[_qp] * 2.0 / 3. * (_u[_qp] / _em[_qp]);

  return -1.0 * _reaction_coefficient[_qp] * _em[_qp] * _target[_qp] * Eel;
}
