...
FVJouleHeating::computeQpResidual()
{
  ...
  interpolate(Moose::FV::InterpMethod::Average, mu, _mu_elem[_qp], mu_neighbor, *_face_info, true);
  ...
  interpolate(
      Moose::FV::InterpMethod::Average, diff, _diff_elem[_qp], diff_neighbor, *_face_info, true);
  ...
  return -grad_potential * _r_units * _voltage_scaling *
         (-mu * -grad_potential * _r_units * em_interface -
          diff * grad_em * em_interface * _r_units);
}
