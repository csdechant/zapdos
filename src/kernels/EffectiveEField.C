...
EffectiveEField::validParams()
{
  ...
  params.addRequiredCoupledVar(
      "ions", "The ions affected by the effective EField.");
  return params;
}
...
EffectiveEField::EffectiveEField(const InputParameters & parameters)
  : ADKernel(parameters),
    ...
    _ions_var(*getVar("ions", 0)),
    _nu(getADMaterialProperty<Real>("nu" + _ions_var.name())),
...
{
  return -_test[_i][_qp] * _nu[_qp] * (-_grad_potential[_qp](_component) - _u[_qp]);
}
