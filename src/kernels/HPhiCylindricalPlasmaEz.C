...
#include "HPhiCylindricalPlasmaEz.h"

registerMooseObject("ZapdosApp", HPhiCylindricalPlasmaEz);

InputParameters
HPhiCylindricalPlasmaEz::validParams()
{
...
HPhiCylindricalPlasmaEz::HPhiCylindricalPlasmaEz(const InputParameters & parameters)
  : ADKernel(parameters),
    ...
    _real_Hphi_value(adCoupledValue("real_Hphi")),
    _real_Hphi_grad(adCoupledGradient("real_Hphi")),
    _imag_Hphi_value(adCoupledValue("imaginary_HPhi")),
...
ADReal
HPhiCylindricalPlasmaEz::computeQpResidual()
{
  ADReal res = 0.0;
  ...
  if (_component == REAL)
  {
    res = _test[_i][_qp] * (_real_Hphi_value[_qp] / _q_point[_qp](0) + _real_Hphi_grad[_qp](0) + omega * _eps0 * (_eps_r_real[_qp] * _imag_value[_qp] + _eps_r_imag[_qp] * _real_value[_qp]));
  }
  else if (_component == IMAGINARY)
  {
    res = _test[_i][_qp] * (_imag_Hphi_value[_qp] / _q_point[_qp](0) + _real_Hphi_grad[_qp](0) - omega * _eps0 * (_eps_r_real[_qp] * _real_value[_qp] - _eps_r_imag[_qp] * _imag_value[_qp]));
  }
