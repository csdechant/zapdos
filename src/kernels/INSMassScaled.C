...
#include "INSMassScaled.h"
#include "Function.h"

registerMooseObject("NavierStokesApp", INSMassScaled);

template <>
InputParameters
validParams<INSMassScaled>()
{
...
INSMassScaled::INSMassScaled(const InputParameters & parameters)
  : INSBase(parameters),
...
Real
INSMassScaled::computeQpResidual()
{
  // (div u) * q
  ...
  Real r = -(_grad_u_vel[_qp](0) + _grad_v_vel[_qp](1) + _grad_w_vel[_qp](2)) * _r_units * _test[_i][_qp];
...
Real
INSMassScaled::computeQpPGResidual()
{
  ...
  Real r = -1. / _rho[_qp] * tau() * _grad_test[_i][_qp] * _r_units *
...
Real
INSMassScaled::computeQpJacobian()
{
...
Real
INSMassScaled::computeQpPGJacobian()
{
  return -1. / _rho[_qp] * tau() * _grad_test[_i][_qp] * _r_units * dStrongPressureDPressure();
}

Real
INSMassScaled::computeQpOffDiagJacobian(unsigned jvar)
{
  if (jvar == _u_vel_var_number)
  {
    Real jac = -_grad_phi[_j][_qp](0) * _r_units * _test[_i][_qp];
    if (_pspg)
  ...
  {
    Real jac = -_grad_phi[_j][_qp](1) * _r_units * _test[_i][_qp];
    if (_pspg)
  ...
  {
    Real jac = -_grad_phi[_j][_qp](2) * _r_units * _test[_i][_qp];
    if (_pspg)
...
Real
INSMassScaled::computeQpPGOffDiagJacobian(unsigned comp)
{
  RealVectorValue convective_term = _convective_term ? convectiveTerm() : RealVectorValue(0, 0, 0);
  ...
  return -1. / _rho[_qp] * tau() * _grad_test[_i][_qp] * _r_units *
             (d_convective_term_d_u_comp + d_viscous_term_d_u_comp + d_transient_term_d_u_comp) -
         1. / _rho[_qp] * dTauDUComp(comp) * _grad_test[_i][_qp] * _r_units *
