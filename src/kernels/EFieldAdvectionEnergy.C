...
EFieldAdvectionEnergy::computeQpResidual()
{
  ...
  {
    return _muel[_qp] * _sign[_qp] * _u[_qp] * -_grad_potential[_qp] * _r_units *
           -_grad_test[_i][_qp] * _r_units;
  }
}
...
EFieldAdvectionEnergy::computeQpJacobian()
{
  ...
  {
    _d_actual_mean_en_d_u = 1.0 / _em[_qp] * _phi[_j][_qp];
    _d_muel_d_u = _d_muel_d_actual_mean_en[_qp] * _d_actual_mean_en_d_u;

    return (_d_muel_d_u * _sign[_qp] * _u[_qp] * -_grad_potential[_qp] * _r_units +
            _muel[_qp] * _sign[_qp] * _phi[_j][_qp] * -_grad_potential[_qp] *
                _r_units) *
           -_grad_test[_i][_qp] * _r_units;
  }
}
...
EFieldAdvectionEnergy::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  else
  {
    if (jvar == _potential_id)
    {
      return _muel[_qp] * _sign[_qp] * _u[_qp] * -_grad_phi[_j][_qp] * _r_units *
             -_grad_test[_i][_qp] * _r_units;
    }

    if (jvar == _em_id)
    {
      _d_actual_mean_en_d_em = _u[_qp] / (_em[_qp] * _em[_qp]) * -_phi[_j][_qp];
      _d_muel_d_em = _d_muel_d_actual_mean_en[_qp] * _d_actual_mean_en_d_em;

      return _d_muel_d_em * _sign[_qp] * _u[_qp] * -_grad_potential[_qp] * _r_units *
             -_grad_test[_i][_qp] * _r_units;
    }
