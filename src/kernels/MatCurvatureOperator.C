...
#include "MatCurvatureOperator.h"

registerADMooseObject("ZapdosApp", MatCurvatureOperator);

InputParameters
MatCurvatureOperator::validParams()
{
  ...
  params.addClassDescription("Contributes the curvature operator multiple by a "
                             "coefficient. (Note: The acting variable needs to be in log form.)");
  return params;
}

MatCurvatureOperator::MatCurvatureOperator(const InputParameters & parameters)
  : ADKernel(parameters),
...
ADReal
MatCurvatureOperator::computeQpResidual()
{
