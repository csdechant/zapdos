...
#include "CoeffPerpendicularDiffusionLin.h"

registerADMooseObject("ZapdosApp", CoeffPerpendicularDiffusionLin);

InputParameters
CoeffPerpendicularDiffusionLin::validParams()
{
  ...
  params.addClassDescription(
      "The magnetic perpendicular diffusion, where the "
      "Jacobian is computed using forward automatic differentiation.");
  return params;
}

CoeffPerpendicularDiffusionLin::CoeffPerpendicularDiffusionLin(const InputParameters & parameters)
  : ADKernel(parameters),
...
ADReal
CoeffPerpendicularDiffusionLin::computeQpResidual()
{

  return -_perp_diffusivity[_qp] * -_grad_test[_i][_qp] *
         (_grad_u[_qp] * _r_units -
          _magnetic_unit_vector[_qp] * (_magnetic_unit_vector[_qp] * _grad_u[_qp] * _r_units));
}
