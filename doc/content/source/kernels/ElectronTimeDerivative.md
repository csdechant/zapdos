...
`ElectronTimeDerivative` is the time derivative term used for variables in log form.
Despite the name, `ElectronTimeDerivative` can be applied to any log form variable,
not just electrons.
...
Where $n_{j}$ is the density. When converting the density to log form, the
