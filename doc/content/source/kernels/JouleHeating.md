...
for electrons and electron mean energy densities in log form. `JouleHeating` assumes
...
Where $V$ is the potential, $\mu_{e}$ is the mobility of the electrons, $n_{e}$ is
the electron density, and $D_{e}$ is the diffusion coefficient. When converting
the density to log form and applying scaling factor to the mesh and voltage,
...
Where $N_{e}$ is the molar density of the electrons in log form, $V_{c}$ is the
