...
if "/zapdos/doc/" in script_folder.lower():     # if in documentation folder
    csv_folder_term01 = "../../../test/tests/mms/magnetized_plasma/continuity_equations/divergence_of_flux/gold/scalarLog_divergenceMatVelocity_product.csv"
    csv_folder_term02 = "../../../test/tests/mms/magnetized_plasma/continuity_equations/divergence_of_flux/gold/matVector_gradentScalarLog_product.csv"
    csv_folder_combined = "../../../test/tests/mms/magnetized_plasma/continuity_equations/divergence_of_flux/gold/divergence_of_flux.csv"
...
ax1.loglog(h_01,u_01, marker='o', linestyle='dashed',color='r',label='$f$ solved with first term only: {:.3f}'.format(u_01_slope[0]))
ax1.loglog(h_02,u_02, marker='o', linestyle='dashed',color='g',label='$f$ solved with second term only: {:.3f}'.format(u_02_slope[0]))
ax1.loglog(h_combined,u_combined, marker='o', linestyle='solid',color='b',label='$f$ solved with total divergence term: {:.3f}'.format(u_combined_slope[0]))
ax1.grid(True, which='both', color=[0.8]*3)
plt.legend(loc='upper left')
fig.set_figwidth(5.5)
fig.set_figheight(6)
fig.savefig('divergence_of_flux_convergence.png', bbox_inches='tight', dpi=300)
