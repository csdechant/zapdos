...
if "/zapdos/doc/" in script_folder.lower():     # if in documentation folder
    csv_folder = "../../../test/tests/mms/magnetized_plasma/model_verification/Hasegawa_Wakatani_model/adiabatic_term/gold/adiabatic_term.csv"
else:                                          # if in test folder
    csv_folder = "./gold/adiabatic_term.csv"
...
ax1.loglog(h,u, marker='o', linestyle='solid',color='b',label='$n$ with an adiabatic source term: {:.3f}'.format(u_slope[0]))
ax1.grid(True, which='both', color=[0.8]*3)
plt.legend(loc='upper left')
fig.set_figwidth(5.5)
fig.set_figheight(6)
fig.savefig('adiabatic_term_convergence.png', bbox_inches='tight', dpi=300)
