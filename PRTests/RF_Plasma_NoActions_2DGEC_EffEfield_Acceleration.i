...
#New Boundary conditions for ions, should be the same as in paper
  [./Ar+_physical_advection]
    type = ADSakiyamaIonAdvectionWithEffEfieldBC
    variable = Ar+
    Ex = Ex
    Ey = Ey
    boundary = 'Top_Electrode Bottom_Electrode Top_Insulator Bottom_Insulator Walls'
...
#New Boundary conditions for mean energy, should be the same as in paper
  [./mean_en_physical_diffusion]
    type = ADSakiyamaEnergyDiffusionBC
    variable = mean_en
    em = em
    boundary = 'Top_Electrode Bottom_Electrode Top_Insulator Bottom_Insulator Walls'
    position_units = ${dom0Scale}
  ...
  [./mean_en_Ar+_second_emissions]
    type = ADSakiyamaEnergySecondaryElectronWithEffEfieldBC
    variable = mean_en
    em = em
    ip = Ar+
    Ex = Ex
...
[Functions]
  [./potential_top_bc_func]
    type = ParsedFunction
    value = '0.030*sin(2*pi*13.56e6*t)'
  [../]
  ...
  [./density_ic_func]
    ...
    value = 'if(y > 1.5 & y < 2.5,
                log((1e10 + (-0.5 * ( tanh((x - 1.9)/0.059) - 1))*(10^15*((2.5-y)^2 * (y - 1.5)^2) + 1e13))/6.02e23),
                log(1e10/6.02e23))'
  [../]
  [./energy_density_ic_func]
    ...
    value = 'if(y > 1.5 & y < 2.5,
                log((3./2.)) + log((1e10 + (-0.5 * ( tanh((x - 2)/0.059) - 1))*(10^15*((2.5-y)^2 * (y - 1.5)^2) + 1e13))/6.02e23),
...
[Materials]
  ...
  [./gas_constants]
    ...
    block = 'plasma'
    prop_names = ' e         N_A     k_boltz  eps         T_gas massem   p_gas  n_gas    NonAD_muAr+'
    prop_values = '1.6e-19   6.02e23  1.38e-23 8.854e-12  300   9.11e-31 113.33 40.4915  144.409938'
  [../]
  [ad_gas_constants]
...
[MultiApps]
  ...
  [./Shooting]
    ...
    input_files = 'RF_Plasma_NoActions_2DGEC_EffEfield_Shooting.i'
    execute_on = 'TIMESTEP_END'
    enable = false
  [../]
[]
