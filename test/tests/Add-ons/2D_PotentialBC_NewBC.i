...
#Source Terms in moles
  ...
  [./energy_left_BC]
    ...
    vars = 'ee N_A diffpotential diffem muem massem diffmean_en diffion muion'
    vals = 'ee N_A diffpotential diffem muem massem diffmean_en diffion muion'
    value = 'log(((3*massem*pi*((diffpotential*pi*cos(pi*t) - cos(pi*t)*sin(pi*y) + 4)/ee + 8*muion*pi*(sin(pi*t) + 1)*(sin(pi*y) + (cos(pi*y)*sin(2*pi*t))/5 + 2))^2)/(16*ee*(sin(pi*y) + (cos(pi*y)*sin(2*pi*t))/5 + 2))) / N_A)'
  [../]
  [./ion_left_BC]
    type = ParsedFunction
    vars = 'ee N_A diffpotential diffem muem massem diffmean_en diffion muion'
  ...
  [./energy_down_BC]
    ...
    vars = 'ee N_A diffpotential diffem muem massem diffmean_en diffion muion'
    vals = 'ee N_A diffpotential diffem muem massem diffmean_en diffion muion'
    value = 'log(((3*massem*pi*((diffpotential*pi*cos(pi*t) - cos(pi*t)*sin(pi*x) + 4)/ee + 8*muion*pi*(sin(pi*t) + 1)*(sin(2*pi*t)/5 + sin(pi*x) + 2))^2)/(16*ee*(sin(2*pi*t)/5 + sin(pi*x) + 2))) / N_A)'
  [../]
  [./ion_down_BC]
    type = ParsedFunction
    vars = 'ee N_A diffpotential diffem muem massem diffmean_en diffion muion'
  ...
  [./em_up_BC]
    ...
    vars = 'ee N_A diffpotential diffem muem massem diffmean_en diffion muion'
    vals = 'ee N_A diffpotential diffem muem massem diffmean_en diffion muion'
    value = 'log((sin(pi*x) - sin(2*pi*t)/5 + 2) / N_A)'
  [../]
  [./energy_up_BC]
    type = ParsedFunction
    vars = 'ee N_A diffpotential diffem muem massem diffmean_en diffion muion'
    vals = 'ee N_A diffpotential diffem muem massem diffmean_en diffion muion'
    value = 'log(((3*massem*pi*((diffpotential*pi*cos(pi*t) - cos(pi*t)*sin(pi*x) + 4)/ee + 8*muion*pi*(sin(pi*t) + 1)*(sin(pi*x) - sin(2*pi*t)/5 + 2))^2)/(16*ee*(sin(pi*x) - sin(2*pi*t)/5 + 2))) / N_A)'
  [../]
  [./ion_up_BC]
    type = ParsedFunction
    vars = 'ee N_A diffpotential diffem muem massem diffmean_en diffion muion'
    vals = 'ee N_A diffpotential diffem muem massem diffmean_en diffion muion'
    value = 'sin(pi*x) - sin(2*pi*t)/5 + 2'
  [../]
  [./Ex_up_BC]
    type = ParsedFunction
    vars = 'ee N_A diffpotential diffem muem massem diffmean_en diffion muion'
...
[Outputs]
  ...
  file_base = '2D_PotentialBC_FluxLeft'
  perf_graph = true
  [./out]
    type = Exodus
  [../]
