#!/usr/bin/env python3
# Script to run spatial convergence study on parallel_velocity.i
...
if "/zapdos/doc/" in script_folder.lower():     # if in documentation folder
    csv_folder = "../../../test/tests/mms/magnetized_plasma/material_velocities_and_unit_vector/parallel_velocity/gold/parallel_velocity.csv"
else:                                          # if in test folder
    csv_folder = "./gold/parallel_velocity.csv"
...
ax1.loglog(h,velocity, marker='o', linestyle='solid',color='b',label='$\\vec v _\\parallel$: {:.3f}'.format(velocity_slope[0]))
ax1.loglog(h,div_velocity, marker='o', linestyle='dashed',color='r',label='$\\nabla \\cdot \\vec v _\\parallel$: {:.3f}'.format(div_velocity_slope[0]))
ax1.grid(True, which='both', color=[0.8]*3)
plt.legend(loc='upper left')
fig.set_figwidth(5.5)
fig.set_figheight(6)
fig.savefig('parallel_velocity_convergence.png', bbox_inches='tight', dpi=300)
