...
#Source Terms in moles
  ...
  #            ee*muion*x^4*sin(2*pi*f*t)*sin(pi*x/l)/(5*diffpotential*l^4)) / N_A'
    type = ParsedFunction
    value = '-diffion*(2*x^2 + 2*x*(2*x - 2) + x*(4*x - 4) + 2*(1 - x)^2)/N_A - 1/5*ee*muion*(x^2*(1 - x)^2 + 1.0)*sin(x*pi)*sin(2*pi*f*t)/(N_A*diffpotential) + (1/5)*ee*muion*(x^2*(2*x - 2) + 2*x*(1 - x)^2)*sin(2*pi*f*t)*cos(x*pi)/(pi*N_A*diffpotential)'
    vars = 'ee diffion muion diffpotential N_A f'
    vals = 'ee diffion muion diffpotential N_A f'
  [../]
  [./potential_source]
  #  type = ParsedFunction
  #  vars = 'l f ee diffpotential'
  ...
  #  value = '-ee*sin(2*pi*f*t)*sin(pi*x/l)/5 - 2*ee*f*l^2*sin(pi*x/l)*cos(2*pi*f*t)/(5*pi*diffpotential)'
    type = ParsedFunction
    value = '-1/5*ee*sin(x*pi)*sin(2*pi*f*t) - 2/5*ee*f*sin(x*pi)*cos(2*pi*f*t)/(pi*diffpotential)'
    vars = 'ee diffpotential f'
    vals = 'ee diffpotential f'
  [../]
